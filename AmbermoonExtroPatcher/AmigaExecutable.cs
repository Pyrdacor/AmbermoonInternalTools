...
public static class AmigaExecutable
{
    ...
    public static void Write(IDataWriter dataWriter, List<IHunk> hunks)
    {
        ...
        foreach (var hunk in realHunks)
        {
            if ((hunk.MemoryFlags & 0x3fffffff) != 0 ||
                ...
                throw new Exception("Invalid hunk memory flags");

            if (hunk.Size > 0x3fffffff)
                throw new Exception("Invalid hunk size");
            ...
            switch (hunk.Type)
            {
                ...
                case HunkType.Data:
                {
                    ...
                    if (dataHunk.Data.Length != dataHunk.NumEntries * 4)
                        throw new Exception("Mismatching NumEntries value and Data length for code/data hunk.");

                    if (dataHunk.NumEntries != hunk.Size)
                        throw new Exception("Mismatching NumEntries value and hunk size for code/data hunk.");
                ...
                case HunkType.BSS:
                {
                    ...
                    if (bssHunk.NumEntries != hunk.Size)
                        throw new Exception("Mismatching NumEntries value and hunk size for BSS hunk.");
                ...
                case HunkType.RELOC32:
                {
                    ...
                    if (dataWriter.Position - start != hunk.Size)
                        throw new Exception("Error writing RELOC32 hunk data");
                    break;
                }
            }
        }
        ...
        if (hunks[^1].Type != HunkType.END)
            dataWriter.Write((uint)HunkType.END);
    }
    ...
    public static List<IHunk> Read(IDataReader dataReader, bool deplodeIfNecessary = true)
    {
        ...
        for (int i = 0; i < numHunks; ++i)
        {
            ...
            switch (type)
            {
                ...
                default:
                    throw new Exception($"Unsupported hunk type: {type}.");
            }
    ...
    static int FindSequence(byte[] data, int offset, params byte[] sequence)
    {
        for (int i = offset; i < data.Length - sequence.Length + 1; i++)
        {
            bool found = true;
    ...
    static List<IHunk> ReadImploded(List<IHunk> imploderHunks)
    {
        ...
        //Console.WriteLine(string.Join(Environment.NewLine, imploderHunks.Select(h => h.Type.ToString())));
        //Environment.Exit(0);

        /*var dataHunks = imploderHunks.Where(h => h.Type == HunkType.Code || h.Type == HunkType.Data).Select(h => ((Hunk)h).Data).ToArray();

        for (int i = 0; i < dataHunks.Length; i++)
        {
            int offset = 0;

            while (true)
            {
                int index = FindSequence(dataHunks[i], offset, 0x03, 0xe9);
            ...
            if (flags == 2 || flags == 3) // BSS or DATA
            {
                if (reader.Position < reader.Size && (reader.PeekDword() & 0x3fffffff) != 0) // a size follows -> no BSS but DATA
                {
                    ...
                    if (hunkSize * 4 != hunkSizes[hunkSizeIndex])
                        throw new Exception("Invalid hunk data size.");
    ...
    public static byte[] Deplode(IDataReader dataReader, out List<uint> deplodedHunkSizes, out List<uint> deplodedMemFlags)
    {
    ...
    static unsafe byte[] Deplode(List<IHunk> imploderHunks, out List<uint> deplodedHunkSizes, out List<uint> deplodedMemFlags)
    {
        ...
        fixed (byte* ptr = &data[0])
        {
            if (!Deploder.Deplode(ptr, buffer, table, dataSize, firstLiteralLength, initialBitBuffer))
                throw new Exception("Invalid imploded data.");
        }
